...
class TestCart (unittest.TestCase):
    ...
    def items(self):
        ...
        return map(lambda jp: instanciator.instantiate(jp), self.cartExample()["items"])
    def setUp(self):
        self.environment = redbubble.Environment(self.products())
        self.cart = redbubble.Cart(self.environment)
        self.cart.country = self.cartExample()['country']
    ...
    def test_cart_item_respects_formule(self):
        formule = lambda item: (item.product().priceFor("Australia") + round(item.product().priceFor("Australia") * item.artist_markup / 100)) * item.quantity
